...
void loadTrainClassFile(string trainclass_files , std::vector<std::vector<string> >& vFilenames, std::vector<std::vector<CvRect> >& vBBox, std::vector<std::vector<CvPoint> >& vCenter, std::vector<string>& internal_files) {
		...
		for(int i=0; i<(int)vFilenames[l].size(); ++i) {
	  		...
	  		if(i%50==0) cout << i << " " << flush;
      			...
      			if(subsamples <= 0 || (int)vFilenames[l].size()<=subsamples || (cvRandReal(pRNG)*double(vFilenames[l].size()) < double(subsamples)) ) {
					...
					img = cvLoadImage((trainclasspath + "/" +  img_file_name).c_str(),CV_LOAD_IMAGE_COLOR);
					// is going to be IPL_DEPTH_16U
					depth_img = cvLoadImage((trainclasspath + "/" + img_depth_file_name).c_str(),CV_LOAD_IMAGE_ANYDEPTH);
		
					if(!img) {
		  				cout << "Could not load image file: " << (trainclasspath + "/" + img_file_name).c_str() << endl;
		  				exit(-1);
					} else if(!depth_img) {
		  				cout << "Could not load image file: " << (trainclasspath + "/" + img_depth_file_name).c_str() << endl;
		  				exit(-1);
					}
...
// FROM HERE THE DETECTION STARTS
			...
			img = cvLoadImage((impath + "/" +  img_file_name).c_str(),CV_LOAD_IMAGE_COLOR);
			depth_img = cvLoadImage((impath + "/" + img_depth_file_name).c_str(),CV_LOAD_IMAGE_ANYDEPTH);
			...
			int tstart = clock();
			vector<vector<IplImage*> > vImgAssign;	
			crDetect.fullAssignCluster(img, depth_img, vImgAssign, scales);
			vector<vector<float> > candidates;
			
			vector<vector<IplImage*> > classConfidence;
			
			crDetect.getClassConfidence(vImgAssign,classConfidence);


			if (false){
				for (unsigned int scNr=0; scNr < classConfidence.size(); scNr++){
					for (unsigned int cNr=0; cNr < classConfidence[scNr].size(); cNr++){
						//int delay = 500;
						cvNamedWindow("class",1);
						cvShowImage("class",classConfidence[scNr][cNr]);
						cvWaitKey();
						cvDestroyWindow("class");
					}
				}
			}
...
//initializing the file for the candidates	
			...
			// Release image
			cvReleaseImage(&img);
			// Release Assign Images
			for (unsigned int i=0; i < vImgAssign.size(); i++){
				for (unsigned int j=0; j< vImgAssign[i].size(); j++)
					cvReleaseImage(&vImgAssign[i][j]);	
			}
			// Release Confidence Images
			for (unsigned int i=0; i < classConfidence.size(); i++){
				for (unsigned int j=0; j< classConfidence[i].size(); j++)
					cvReleaseImage(&classConfidence[i][j]);
			}
		}
	}
}
