...
void CRPatch::extractPatches(IplImage *img, IplImage *depth_img, unsigned int n, int label, CvRect* box, CvPoint* vCenter, float scale) {
	// extract features
	...
	CvMat* locations = cvCreateMat( (img->width-width)*(img->height-height), 1, CV_32SC2 );// n // get more random
	if(box==0)
	...
	for(unsigned int i=0; count < n && i < (img->width-width)*(img->height-height); ++i) {
...
void CRPatch::extractPatches(IplImage *img, IplImage *depth_img, unsigned int n, int label, int imageID, CvRect* box, CvPoint* vCenter, float scale) {
	// extract features
	vector<IplImage*> vImg;
	extractFeatureChannels(img, depth_img, vImg, scale);
	...
	// generate x,y locations
	CvMat* locations = cvCreateMat( (img->width-width)*(img->height-height), 1, CV_32SC2 );
	if(box==0)
		cvRandArr( cvRNG, locations, CV_RAND_UNI, cvScalar(0,0,0,0), cvScalar(img->width-width,img->height-height,0,0) );
	else
		cvRandArr( cvRNG, locations, CV_RAND_UNI, cvScalar(box->x,box->y,0,0), cvScalar(box->x+box->width-width,box->y+box->height-height,0,0) );
...
void CRPatch::extractFeatureChannels(IplImage *img, IplImage *depth_img, std::vector<IplImage*>& vImg, float scale) {
	// 32 feature channels
	...
	
	{
	  ...
	  for( y = 0; y < size.height; y++, dataX += stepX, dataY += stepY, dataZ += stepZ  )
	    for( x = 0; x < size.width; x++ ) {
	    	/*float magf = sqrt((float)dataX[x]*(float)dataX[x] + (float)dataY[x]*(float)dataY[x]);
	    	uchar mag = (uchar)(magf);
	    	if(magf > mag+1) {
	    		std::cout << magf << " -> " << (int)mag << std::endl;
	    		throw "dat overflowt doch!";
	    	}*/
	...
	cv::Mat vImg16(vImg[16]);
	cv::Mat vImg17(vImg[17]);
	cv::Mat vImg18(vImg[18]);
	cv::Mat vImg19(vImg[19]);

	vImg16 = cv::Scalar(0);
	convertScaleAbs(depth_img_mat / scale, vImg17, 0.075);
	...
	convertScaleAbs(dI_x, vImg18, 1);
	dI_y = abs(dI_y);
	dI_y *= 5;
	threshold(dI_y, dI_y, 255, 255, CV_THRESH_TRUNC);
	convertScaleAbs(dI_y, vImg19, 1);

	for(int c=20; c<29; ++c) {
		cv::Mat vImg_mat(vImg[c]);
		vImg_mat = cv::Scalar(0);
	}
